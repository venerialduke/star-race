...
Things a player adds to the track itself: a new route through a sector, or a
whole new sector. Permanent or temporary. Only players in the same heat see
an augmented track. Round 7 called the route kind a _player split_; the
...
| `adds`    | `route` — a new way through an existing sector · `sector` — a new stretch with its own checkpoint |
...
| ---------- | ------------ | ------ | ----------------------------------------------------------------------------- | ------ | --------- | ---- | ------ |
| add-route  | Add a route  | route  | A new way through a sector you choose, so the sector splits where it did not. | either | same heat |      | stub   |
| add-sector | Add a sector | sector | A whole new sector spliced into the loop, with its own checkpoint.            | either | same heat |      | stub   |

## Notes

**2026-09-13.** Round 7 has two named route augments — a _pocket split_ that
