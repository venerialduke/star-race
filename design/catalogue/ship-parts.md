...
| collector-shield      | Collector shield        | shields    | A weaker shield than the plain one, which doubles as a dark matter collector.                                                                                                    | `Shields↑ charge↓`          | 1                   | **L2** shields a little better but recharges slowly, and collects dark matter — less than a level 1 dark matter collector. **L3** collects better, and adds an ability: a weapon or moving fixture that hits the ship at full shields is **collected**, and collected fixtures sell for credits at the end of the race. Storage capacity scales with the ship's total shield capacity, so more shields fitted means more room. |       | dark matter engine, a second shield |        | seeded |
...
**2026-09-13, the collector shield, and what the table still wants.** One more
shields row, and a standing to-do: **more basic engines, shields and
navigation systems are wanted** — plain ones, the kind a player starts with.
Every component so far has a hook; nothing is yet merely competent. This is
what the frames question below hangs on: a frame only means something if there
is a spread of ordinary parts for it to be a selection from.
